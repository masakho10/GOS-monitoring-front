...
const EnteteFacture: React.FC = () => {
  const [data, setData] = useState<EnteteFactureData[]>([]);
  //const navigate = useNavigate();

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<EnteteFactureData[]>("http://localhost:3001/api/entete-facture");
