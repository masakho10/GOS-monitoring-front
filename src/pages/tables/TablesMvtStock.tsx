...
const MvtStock: React.FC = () => {
  const [data, setData] = useState<MvtStockData[]>([]);
  //const navigate = useNavigate();

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<MvtStockData[]>("http://localhost:3001/api/mvt-stock");
