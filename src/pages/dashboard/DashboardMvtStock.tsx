...
const DashboardMvtStock = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const successRes = await axios.get('http://localhost:3001/api/mvt_stock_success_log');
        const errorRes = await axios.get('http://localhost:3001/api/mvt_stock_error_log');
        const rateRes = await axios.get('http://localhost:3001/api/mvt_stock_success_percentage');
  ...
  const handleNavigateToTables = () => {
    navigate("/tables-mvt-stock");
  };

  return (
    <>
      <div>
        <h2>Dashboard Mvt Stock</h2>
      </div>
...
export default DashboardMvtStock;
