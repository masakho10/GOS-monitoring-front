...
const DashboardPaiements = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const successRes = await axios.get('http://localhost:3001/api/paiements_success_log');
        const errorRes = await axios.get('http://localhost:3001/api/paiements_error_log');
        const rateRes = await axios.get('http://localhost:3001/api/paiements_success_percentage');
  ...
  const handleNavigateToTables = () => {
    navigate("/tables-paiements");
  };

  return (
    <>
      <div>
        <h2>Dashboard Paiements</h2>
      </div>
      <View borderRadius="6px" maxWidth="100%" padding="0rem" minHeight="100vh">
        ...
        >
          ...
          <HoverableDiv style={{ gridRow: 'span 1' }} onClick={handleNavigateToTables}>
            <MiniStatistics
              title="Error Log"
          ...
          <HoverableDiv style={{ gridRow: 'span 4' }} onClick={handleNavigateToTables}>
